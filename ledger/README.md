...
- **Disagreement is preserved, not merged.** A superseded observation is corrected by appending a new entry on the same `about`; the old line is never edited.
  ...
  **A skipped check reports differently depending on whether it could have run.** Outside a checkout — a release tarball, a vendored copy — there is no history and never was, so the gate prints `DOES NOT APPLY` and passes; failing there would report a missing feature as a contract violation. Inside a checkout the same skip is an error, because the strongest half of the gate did not run and passing would make it optional. The two are told apart by whether `.git` exists, which is a filesystem question on purpose: asking git would share the failure mode of the call being diagnosed, and a probe that fails whenever its subject does separates nothing. `scripts/validate-observations.test.mjs` covers both sides, and substituting a `git rev-parse` probe for the filesystem one turns a test red.
