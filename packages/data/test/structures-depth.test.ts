...
const ALL_DEPTH = [...CRITICAL_FAMILY_DEPTH, ...INFERENCE_FAMILY_DEPTH, ...COLLECTIVE_FAMILY_DEPTH, ...LOCKIN_FAMILY_DEPTH, ...METHOD_FAMILY_DEPTH];
...
describe('structure depth', () => {
  it('lands on the structures each family claims, and only those', () => {
    ...
    expect(withDepth).toHaveLength(40);
  ...
  it('gives the connective tissue the mapping layer never did', () => {
    ...
    expect(edges.length).toBeGreaterThanOrEqual(84);
