...
describe('fallback data matches the prototype', () => {
  it('has 20 seeded isles', () => {
  ...
  it('marks one dormant and one outlier island', () => {
    expect(DATA.filter((d) => d.dor)).toHaveLength(1);
    expect(DATA.filter((d) => d.out)).toHaveLength(1);
    expect(DATA.find((d) => d.dor)!.n).toBe('玻璃之问');
    expect(DATA.find((d) => d.out)!.n).toBe('梦的回收');
  });

  it('spot-checks prototype values (high-temp superconductor #6)', () => {
    const hi = DATA.find((d) => d.id === 6)!;
    expect(hi.n).toBe('高温超导');
    expect(hi.st).toBe(3);
    expect(hi.m).toBe(14);
    expect(hi.a).toBe(88);
  });
