...
export default function AtlasChartHost({ islands, onPick, onHoverIsland, onWebglError }: AtlasChartHostProps) {
  ...
  const cbRef = useRef({ onPick, onHoverIsland, onWebglError });
  cbRef.current = { onPick, onHoverIsland, onWebglError };
  ...
  useEffect(() => {
    ...
    stage.onHover = (slug) => {
      ...
      const s = stage.scale;
      cbRef.current.onHoverIsland(d, { x: d.x * s + stage.worldRoot.x, y: d.y * s + stage.worldRoot.y });
    };

    void stage
      .init(host, { resizeTo: host })
      .then(() => {
        if (disposed) {
          stage.destroy();
          return;
