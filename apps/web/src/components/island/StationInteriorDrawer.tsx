...
export function StationInteriorDrawer({ character, station, plan, lang, islandTitle, question, followingQuestion=false, personalNote='', onPersonalNote,
  ...
  return <>
    ...
    <div ref={dialogRef} className="fi-interior-tower fi-building-interior" data-open="true" data-screen-label="L2 站点内景抽屉" data-station={station}
      ...
      {question && station!=='questions' && <details className="fi-building-question" open={followingQuestion&&!compact}><summary>{followingQuestion?(zh?'带来的具体追问':'The question I am following'):(zh?'我从这个问题而来':'The question I came with')}{followingQuestion&&compact&&<span className="fi-carried-preview">{question}</span>}</summary><p>{question}</p>{followingQuestion&&station==='library'&&<small className="fi-question-source-scope">{zh?'这里展示本岛收录的文献，尚未逐条建立与这条追问的对应。':'These are island-level references; a specific link to this question has not been established.'}</small>}</details>}
      <div className="fi-building-layout">
        ...
        <section ref={readingRef} className="fi-building-reading" onScroll={e=>{if(selected)readingPositions?.set(selected.id,e.currentTarget.scrollTop);}} role={selected ? "tabpanel" : undefined} id={selected ? `room-panel-${rooms.indexOf(selected)}` : undefined} aria-labelledby={selected ? `room-tab-${rooms.indexOf(selected)}` : undefined} tabIndex={0}>
          {selected ? <>
            {plan&&<details className="fi-reader-plan"><summary>{zh?'展开建筑研究结构':'Explore this building’s research spaces'}</summary><BuildingPlan plan={plan} lang={lang} selected={selected.id} onRoom={id=>choose(rooms.findIndex(room=>room.id===id))}/></details>}
