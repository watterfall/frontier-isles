 ...
 *  item-0 transplant 移栽 flow (choose 实验坊 / 白板厅). */
export function DriftwoodModal({ onClose, driftDest, transTo, onMove, onToWorkshop, onToCanvas }: DriftwoodModalProps) {
  ...
  return (
    <div>
      ...
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', width: 460, background: '#FAF5E8', border: '1.5px solid #3A342B', borderRadius: 8, boxShadow: '0 24px 60px rgba(24,20,14,0.4)', padding: '20px 22px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            ...
            <div style={{ fontFamily: "'Noto Serif SC',serif", fontWeight: 900, fontSize: 20, color: '#2B2620', marginTop: 3 }}>{t('island.drift.title')}</div>
          </div>
          <div onClick={onClose} style={{ cursor: 'pointer', width: 28, height: 28, border: '1.5px solid #3A342B', borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#2B2620', fontSize: 13, background: '#F2EAD8' }}>✕</div>
        </div>
              ...
              <div key={i} style={{ border: '1px solid rgba(58,52,43,0.35)', background: '#FDFAF1', borderRadius: 6, padding: '10px 14px' }}>
                ...
                {idle && (
                  <div style={{ marginTop: 8 }}>
                    <span onClick={onMove} style={{ cursor: 'pointer', fontSize: 11, padding: '3px 12px', borderRadius: 5, background: '#E3A93C', color: '#3A2E14', border: '1px solid #8A6A1E', userSelect: 'none' }}>{t('island.drift.move')}</span>
                  </div>
                )}
                {choosing && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8, fontSize: 11, color: '#6B6154' }}>
                    {t('island.drift.towards')}
                    <span onClick={onToWorkshop} style={{ cursor: 'pointer', padding: '3px 12px', borderRadius: 5, border: '1.2px solid #B5673A', color: '#B5673A', userSelect: 'none' }}>{t('island.drift.workshop')}</span>
                    <span onClick={onToCanvas} style={{ cursor: 'pointer', padding: '3px 12px', borderRadius: 5, border: '1.2px solid #3E9B7E', color: '#2B7A5F', userSelect: 'none' }}>{t('island.drift.canvas')}</span>
                  </div>
                )}
        ...
        <div style={{ marginTop: 12, fontSize: 10, color: '#A89C88', fontFamily: "'JetBrains Mono',ui-monospace,monospace", textAlign: 'center' }}>{t('island.drift.footer')}</div>
      </div>
    </div>
  );
}
