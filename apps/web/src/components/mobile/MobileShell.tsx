...
export function resolveMobileIslandId(islands: readonly IslandDatum[], slug?: string | null): number | null {
  return islands.find((island) => island.slug === slug)?.id ?? islands[0]?.id ?? null;
}
...
export function MobileShell({ islands, initialIslandSlug = null, modelRuns = [], onRecordModelRun = () => {}, missionRuns = [], onRecordMissionRun, worldTrailEnabled = true, islandNotes = {}, onIslandNote }: MobileShellProps) {
  ...
  useEffect(() => {
    if (!initialIslandSlug) {
      handledDeepLink.current = null;
      return;
    }
    if (handledDeepLink.current === initialIslandSlug) return;
    const island = islands.find((item) => item.slug === initialIslandSlug);
    if (!island) return;
    const hierarchy = mobileHierarchy.get(island.id);
    handledDeepLink.current = initialIslandSlug;
    setSelectedId(island.id);
    setExpandedAnchor(hierarchy?.role === 'satellite' ? hierarchy.parentSlug ?? null : hierarchy?.slug ?? null);
    setQuery('');
    setAltitude(null);
    setSeg('chart');
  }, [initialIslandSlug, islands, mobileHierarchy]);
  ...
  if (onIsland?.slug) return <main className="fi-mobile-island-shell" data-theme={islandNight ? 'night' : 'day'}>
    <Suspense fallback={<p role="status">{t('island.loading')}</p>}>
      <div inert={islandModel} aria-hidden={islandModel || undefined}><GeneratedIslandScreen key={onIsland.slug} slug={onIsland.slug} onVoyageToIsland={slug => { const target=islands.find(island=>island.slug===slug); if(target){selectIsland(target);setOnIsland(target);setIslandToast('');window.history.replaceState(null,'',`#island=${encodeURIComponent(slug)}`);window.scrollTo(0,0);} }} night={islandNight} onToggleNight={() => setIslandNight(value => !value)} onBack={() => { setOnIsland(null); setIslandToast(''); }} onStation={() => {}} actor="local-reader" readOnly onToast={setIslandToast} personalNote={islandNotes[onIsland.slug] ?? ''} onPersonalNote={onIslandNote ? text => onIslandNote(onIsland.slug!, text) : undefined} onOpenModel={() => setIslandModel(true)} /></div>
        ...
        {seg === 'connections' ? (
          ...
          <>
            <div className="fi-mobile-map">
              <svg viewBox="0 0 354 258" role="group" aria-label={t('mobile.chartHint')}>
...
const MOBILE_CONNECTION_COPY = {
  zh: {
    ...
    themeContinue: '以这个问题继续', themeStatusGap: '尚无可靠映射', themeStatusOne: '1 个研究落点', themeStatusMany: '{{count}} 个研究落点', themeReadingMark: '已写正文 · {{fields}} 个学科 · {{relations}} 条关系', themeReadingCount: '{{written}}/{{all}} 已写正文', themeReadingAll: '{{all}} 个主题',
  ...
  en: {
    ...
    themeContinue: 'Continue with this problem', themeStatusGap: 'No reliable mapping yet', themeStatusOne: '1 research landing', themeStatusMany: '{{count}} research landings', themeReadingMark: 'written up · {{fields}} fields · {{relations}} relations', themeReadingCount: '{{written}}/{{all}} written up', themeReadingAll: '{{all}} topics',
...
function MobileConnectionField({ field, lang, carriedQuestion, onCarriedApplied }: { field: ConnectionField | null; lang: 'zh' | 'en'; carriedQuestion: string; onCarriedApplied: () => void }) {
  ...
  // While some topics are still one sentence the ratio is the useful number;
  // once none are, N/N says nothing and the plain count is the honest line.
  const themeRatio = (items: ConnectionField['topics']): string => {
    const written = items.filter((item) => item.reading).length;
    return written === items.length
      ? copy.themeReadingAll.replace('{{all}}', String(items.length))
      : copy.themeReadingCount.replace('{{written}}', String(written)).replace('{{all}}', String(items.length));
  };
      ...
      {item.reading && (
        <span className="fi-connection-topic-reading">
          ...
          {copy.themeReadingMark
            .replace('{{fields}}', String(item.reading.fields))
            .replace('{{relations}}', String(item.reading.relations))}
        </span>
      )}
    </small>
  );
          ...
          <section className="fi-mobile-connection-starters">
            ...
            {remainingThemes.length > 0 && (
              <details className="fi-mobile-theme-more">
                <summary>{copy.moreThemes}<small>{themeRatio(remainingThemes)}</small></summary>
