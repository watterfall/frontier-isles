...
describe('computeGhostReveals', () => {
  ...
  it('derives all three thresholds from real trigger events, rescaled onto the legacy 1..86 axis', () => {
    const events = [
    ...
    expect(out).toEqual([
      { type: 'card', threshold: Math.round((2 / 5) * 86), source: 'ledger' }, // 34
      { type: 'prototype', threshold: Math.round((3 / 5) * 86), source: 'ledger' }, // 52
      { type: 'canvas', threshold: Math.round((5 / 5) * 86), source: 'ledger' }, // 86
    ]);
    expect(out.every((g) => g.threshold >= 1 && g.threshold <= 86)).toBe(true);
  });
