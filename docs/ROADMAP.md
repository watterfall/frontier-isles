...
6. **Acceptance (§8):** one island runs AI night shift → morning report → bridge → transplant → claim → publication, every step ledger-traceable.
   **Status 2026-07-11:** night shift → morning report → adopt → transplant all run LIVE on machine-curiosity (scout paper → library, every event hash-chained); claim/publication verbs work via gateway with evidence enforcement but haven't been exercised live in one continuous chain yet — the remaining gap to tick §8 is a live submit_claim (with evidence) + publish pass.
