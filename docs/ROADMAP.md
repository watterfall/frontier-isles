...
11. **Canvas scaling / information hierarchy at scale** — the chart is a single fixed SVG (1440×900) with hand-placed coordinates, no zoom/pan, no LOD/culling; islands already overlap at 27 and the practical ceiling is ~mid-30s. When there are many islands the info presentation itself breaks (labels collide, 700 read as loose dots). The scaling plan (deterministic de-overlap → zoom/pan → LOD + variance-select culling → My Harbor + 雾化 fog → PixiJS beyond ~100 islands) is the P3 atlas track — see Phase C. **Unified as one semantic-LOD principle in `docs/scene-upgrade/INFO-HIERARCHY.md`** (info shown = f(zoom, density, variance-select), applied at every scale L0↔L1↔L2); the **L1 station-label LOD already shipped** (crisp billboard labels, seal-glyph→full-name by zoom, in `renderer/pixi/scene-stage.ts`) is the first working instance of that pattern, which Phase C2 extends to the atlas.
12. **Richness followed v2, skipped v1.** The build shipped v2 (horizontal sea-as-data) but never v1's vertical world (sky / sea / seabed = epistemic groundedness), terrain-fingerprint procgen, ritual moments (river lantern 河灯 on publish / storm on dispute / rainbow on resolution / ~8s transplant walk), drift bottle 漂流瓶, resident tree 居民之树, citation birds 候鸟, or My Harbor 我的港湾. Future "more fun / more inviting" richness should re-anchor on `depth-plan-v1.md` — homepage decluttering alone cannot supply it. Like v2, v1 folds into `architecture.md` on ratification (§5.6). **Campaign plan: `docs/richness-plan.md` (2026-07-10)** — note `depth-plan-v1.md` was referenced but never existed; reconstructing it is swimlane A of that campaign.
