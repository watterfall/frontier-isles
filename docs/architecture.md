...
- **Claims & evidence**: claims are first-class artifacts (slender research buildings in the archive court before the Gallery); foundation = public claim, each replication ref adds one storey, consensus adds a roof, and refuted/returned structures survive as night scaffolds. `refute/validate` events must reference an evidence-role data ref; a replication ref is one countable reproduction.
