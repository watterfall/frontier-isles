...
3. **L1 scene rendering:** the prototype is hand-drawn SVG. For pixel-fidelity the L1 scene is data-driven React SVG (`packages/assets` components positioned via `packages/renderer` iso math). The PixiJS 8 engine lives in `packages/renderer` with the full §2 contract + tests, to take over at atlas scale (P3, 700+ islands). This honors "match rendered output; do not port internals".
